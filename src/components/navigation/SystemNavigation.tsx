        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <a
              ...
              className={`group flex items-center gap-2 rounded-full px-3 py-2 text-[9px] tracking-[0.16em] transition-all ${active ? "bg-white/15 text-[#f2f0ea]" : "text-[#77736d] hover:bg-white/8 hover:text-[#f2f0ea]"}`}
      ...
      <div className="mobile-nav-dock fixed bottom-3 left-3 right-3 z-40 flex items-center justify-between rounded-xl border border-white/10 bg-[#0a0a0b]/90 p-1.5 backdrop-blur-xl lg:hidden">
        ...
        <div className="flex items-center gap-1 overflow-x-auto">
          {NAV_ITEMS.filter((item) => MOBILE_PRIMARY_IDS.includes(item.id)).map((item) => (
            <a key={item.id} href={`#${item.id}`} onClick={scrollTo(item.id)} className={`whitespace-nowrap rounded-lg px-2.5 py-2 text-[8px] tracking-[0.12em] transition-colors ${activeSection === item.id ? "bg-white/15 text-[#f2f0ea]" : "text-[#77736d]"}`}>
        ...
        <div className="relative flex items-center gap-1">
          ...
          {isMenuOpen && <div className="mobile-nav-menu absolute bottom-[calc(100%+0.65rem)] right-0 grid min-w-[10rem] gap-1 rounded-xl border border-white/10 bg-[#0a0a0b]/95 p-2 shadow-2xl backdrop-blur-xl">{NAV_ITEMS.filter((item) => !MOBILE_PRIMARY_IDS.includes(item.id)).map((item) =>              <a key={item.id} href={`#${item.id}`} onClick={(e) => { setIsMenuOpen(false); scrollTo(item.id)(e); }} className={`rounded-lg px-3 py-2.5 text-[9px] tracking-[0.14em] transition-colors ${activeSection === item.id ? "bg-white/15 text-[#f2f0ea]" : "text-[#aaa39a] hover:bg-white/8"}`}>{item.code} / {item.label}</a>)}</div>}
        </div>
      </div>
    </>
  );
